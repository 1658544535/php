...
class Model{
	 ...
	 *
	 */
	public function get($Where=array(), $fields='*', $outputType=OBJECT, $paramTable='')
	{
		$sql = $this->getSQL(  $Where, $fields, '', '', $paramTable );
	 ...
	 * @return array
	 */
	public function gets($Where=array(), $fields='*', $order=array(), $page=1, $perpage=20,  $paramTable='')
	{
		$sql = $this->getSQL( $Where, $fields, $order, '', $paramTable );
	 ...
	 * @return integer|boolean false失败
	 */
	public function modify($data, $Where, $table='')
	{
		if(empty($data))
		{
			return false;
		...
		else
		{
			...
			$sql = "UPDATE `{$table}` SET ".implode(',', $sqlSet).' WHERE 1=1'.$this->getSqlWhere($Where, true);
	...
	public function delete($Where, $table='')
	{
		...
		{
			empty($table) && $table = $this->table;
			$sql = "DELETE FROM `{$table}` WHERE 1=1 ".$this->getSqlWhere($Where, true);
	 ...
	 * @return array
	 */
	public function getAll( $Where=array(), $fields='*', $order=array(), $limit='', $output = OBJECT, $paramTable='')
	{
		$sql = $this->getSQL( $Where, $fields, $order, $limit, $paramTable );
	 ...
	 * @return int  :记录数
	 * */
	public function getCount($Where=array(),$paramTable='')
	{
		if ( empty( $paramTable ) )
		{
			$strTable = $this->table;
		...
		$sql = 'SELECT count(*) FROM `'. $strTable .'` WHERE 1=1';
		$sqlWhere = $this->getSqlWhere($Where, true);
	 ...
	 * */
	 public function query( $sql='',$getOne=false, $isPage=false, $page=1, $perpage=20 )
	 {
		...
		$option = strtoupper(substr(trim($sql),0,6));
	...
	{
		$lastQuery = $this->conn->last_query;
		return $lastQuery;
	}
	...
	{
		return $this->conn->debug();
	}

	public function escape($subVal)
	{
		return $this->conn->escape($subVal);
	}
