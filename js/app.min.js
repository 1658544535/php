$(function() {
            	...
            	success: function(res){
		            switch (type){
		            	...
		            	case 'class':
		            		...
		            		if(!!proData["DataSet"]){
		            			for(var i=0; i<proData["DataSet"].length; i++){
				            		html += '<li><a href="'+ proData["DataSet"][i]["id"] +'">'
				                         +       '<div class="img"><img src="'+ proData["DataSet"][i]["image_small"] +'" /></div>'
				                         +       '<div class="name">'
    ...
    $(document).on("pageInit", "#page-freeList", function(e, pageId, page) {
    	var pageNow = $(".freeList").attr("data-pageNow"),
    		pageCount = $(".freeList").attr("data-pageCount"),
    		url = $(".freeList").attr("data-href");

    	getData(pageNow, pageCount, url);

    	// 注册'infinite'事件处理函数
        $(page).on('infinite', function() {
        	pageNow = $(".freeList").attr("data-pageNow"),
    		pageCount = $(".freeList").attr("data-pageCount"),
    		url = $(".freeList").attr("data-href");
        	...
        	$.ajax({
            	...
            	success: function(res){
				    ...
				    for(var i=0; i<res.data.listData.length; i++){
				    	html += '<li><a href="'+ res.data.listData[i]["id"] +'">'
		                     +      '<div class="img"><img src="'+ res.data.listData[i]["imgSrc"] +'" /></div>'
		                     +      '<div class="info">'
		                     +          '<div class="name">'+ res.data.listData[i]["name"] +'</div>'
		                     +          '<div class="price">'
		                     +              '<div class="btn">免费开团</div>'
		                     +              '￥<span class="price1">'+ res.data.listData[i]["oldPrice"] +'</span>/件'
		                     +              '<span class="price2">拼团价：￥'+ res.data.listData[i]["price"] +'</span>'
		                     +          '</div>'
		                     +      '</div>'
		                     +  '</a></li>';
				    }
				    
				    $(".freeList>.list-container").append(html);
		            //容器发生改变,如果是js滚动，需要刷新滚动
		            $.refreshScroller();
		        	...
		        	$(".freeList").attr("data-pageNow", pageNow+1);
		    		$(".freeList").attr("data-pageCount", pageCount);
    ...
    $(document).on("pageInit", "#page-proList", function(e, pageId, page) {
    	var pageNow = $(".index-pro").attr("data-pageNow"),
    		pageCount = $(".index-pro").attr("data-pageCount"),
    		url = $(".index-pro").attr("data-href");

    	getData(pageNow, pageCount, url);

    	// 注册'infinite'事件处理函数
        $(page).on('infinite', function() {
        	pageNow = $(".index-pro").attr("data-pageNow"),
    		pageCount = $(".index-pro").attr("data-pageCount"),
    		url = $(".index-pro").attr("data-href");
        ...
        function getData(pageNow, pageCount, url){
		    ...
		    for(var i=0; i<res.data.listData.length; i++){
		    	html += '<li><a href="'+ res.data.listData[i]["id"] +'">'
                     +       '<div class="img"><img src="'+ res.data.listData[i]["imgSrc"] +'" /></div>'
                     +       '<div class="name">'
                     +           '<span class="num">'+ res.data.listData[i]["num"] +'人团</span>'+ res.data.listData[i]["name"]
                     +       '</div>'
                     +       '<div class="info">'
                     +           '￥<span class="price">'+ res.data.listData[i]["price"] +'</span>'
                     +           '<span class="sales">已团'+ res.data.listData[i]["sales"] +'件</span>'
                     +       '</div>'
                     +   '</a></li>';
		    }
		    $(".index-pro>.list-container").append(html);
            //容器发生改变,如果是js滚动，需要刷新滚动
            $.refreshScroller();
            ...
            pageCount = res.data.pageCount,
			pageNow = res.data.pageNow;
        	$(".index-pro").attr("data-pageNow", pageNow+1);
    		$(".index-pro").attr("data-pageCount", pageCount);
