...
$(function() {
    ...
    //返回首页
    if($("#goIndex").length<=0 && $("input[name='noGoIndex']").val() != 1){
   ...
   //  	});
    ...
    $(document).on("pageInit", "#page-taskList", function(e, pageId, page) {
        ...
        $(".search").on("click", function(){
            ...
            reqData.type2 = null;
            reqData.type3 = null;
            $("#class-picker,#class-picker-value,#class1,#class2,#class3").val("")
        ...
        var newClassData = [],
            newTwoData = [],
            newThereeData = [];
        for(var one in classData){
            var twoData = classData[one]["twoLevelList"];
            newTwoData = [];
            for(var two in twoData){
                var threeData = twoData[two]["threeLevelList"];
                newThereeData = [];
                for(var three in threeData){
                    newThereeData.push({
                        id: threeData[three]["threeId"],
                        name: threeData[three]["threeName"]
                    });
                };
                newTwoData.push({
                    id: twoData[two]["twoId"],
                    name: twoData[two]["twoName"],
                    child: newThereeData
                });
            }
            newClassData.push({
                id: classData[one]["oneId"],
                name: classData[one]["oneName"],
                child: newTwoData
            });
        }
        var area = new LArea();
        area.init({
            'trigger': '#class-picker',//触发选择控件的文本框，同时选择完毕后name属性输出到该位置
            'valueTo':'#class-picker-value',//选择完毕后id属性输出到该位置
            'keys':{id:'id',name:'name'},//绑定数据源相关字段 id对应valueTo的value属性输出 name对应trigger的value属性输出
            'type':1,//数据源类型
            'data':newClassData//数据源
        });
        area.callback = function(){
            var values = $("#class-picker").val(),
                ids = $("#class-picker-value").val();
            var arrValue = values.split(','),
                arrId = ids.split(',');
            $("#class1").val(arrValue[0]);
            $("#class2").val(arrValue[1]);
            $("#class3").val(arrValue[2]);
            reqData.type1 = arrId[0];
            reqData.type2 = arrId[1];
            reqData.type3 = arrId[2];
        };
        	...
        	success: function(res){
        		...
        		if(res.code>0){
			        ...
			        if($(".page-group").attr("id") == 'page-guessList'){
						...
						ifLoad = res.data.ifLoad,
						pageNow = parseInt(res.data.pageNow);
					}
