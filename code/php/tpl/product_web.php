						...
						<?php foreach($productList as $key=>$product){ ?>
							<li>
								<div class="product_block_main">
									<a class="p_img_warp" href="/product_detail?pid=<?php echo $product->id;?>" pitem="<?php echo $product->id;?>" onclick="preShow(this)" curpage="1">
										<?php if ( $act=="top" && $key < 10 ){ ?>
											<img src="/images/product/top_<?php echo $key+1; ?>.png" alt="" width='20%' class="tip" />
										<?php } ?>
										<div class="square_img"><img class="lazyload" data-original="<?php echo $site_image?>product/small/<?php echo $product->image;?>" /></div>
										<p>
											<?php
			                        ...
			                        <div class="p_desc_warp">
			                           <span>￥<?php echo number_format(empty($product->distribution_price) ? $product->distribution_price : $product->distribution_price,1);?></span>
			                           <!-- <a href="/product_detail?type=<?php echo $type; ?>&pid=<?php echo $product->product_id;?>" pitem="<?php echo $product->product_id;?>" onclick="preShow(this)" curpage="1">立即购买</a> -->
			                        </div>
								</div>
							</li>
						<?php } ?>
