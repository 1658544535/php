        	...
        	success: function(res){
        		...
        		if(res.code>0){
        			...
        			if($("#errorTips").length<=0){
        				$(".pullbox>.list-container").append('<div id="errorTips">'+ res.msg +',请刷新重试</div>');
        			}else{
        				$("#errorTips").html(res.msg +',请刷新重试');
        			}
        			// 加载完毕，则注销无限加载事件，以防不必要的加载
		            $.detachInfiniteScroll($('.infinite-scroll'));
		            // 删除加载提示符
...
function fn_pull_tab(page, fn){
	...
	$(".user-tab li").attr("data-pageCount", pageCount);
    	...
    	xhr = $.ajax({
        	...
        	success: function(res){
        		...
        		if(res.code>0){
					...
					$(".user-tab li.active").attr("data-pageCount", pageCount);
