...
class PayNotifyCallBack extends WxPayNotify
{
    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        //获取订单的相关数据
        list($orderId, $orderNo) = explode('|', $data['attach']);

        $Order = new OrderModel($db, 'orders');
        //处理本系统订单
        $order = $Order->get(array('order_number'=>$orderNo));

        if(empty($order)){
            Log::DEBUG('没有相关订单：'.$orderNo);
            $msg = '没有相关订单';
            return false;
        }

        if($data['total_fee'] != (($order->pay_online - $order->discount_price - $order->discount_integral_price)*100)){
            Log::DEBUG('订单支付金额不一致：订单应付金额为'.$order->pay_online.'，支付金额为'.($data['total_fee']/100));
            $msg = '订单支付金额不一致';
            return false;
        }
        ...
        if($order->is_pay == 0){
            ...
            //更改订单状态
            $success = $Order->modify(array('is_pay'=>1,'paid_price'=>$data['total_fee']/100), array('order_id'=>$order->order_id)) ? true : false;
