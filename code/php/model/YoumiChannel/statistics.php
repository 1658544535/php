...
require_once( MODEL_INC .'AdSpreadModel.class.php' );
...
class Index
{
	...
	public function __construct( $db,$WEB_SIZE )
	{
		...
		$this->AdSpreadModel = new AdSpreadModel($db);
		...
		switch( $type )
		{
			case 'list':
				...
				$get_count = $this->SysLoginModel->getAll(  array( 'reg_channel' => $userinfo->Name ) );
		 		...
		 		//$sql = "SELECT DATE_FORMAT( `create_date`, '%d' ) AS day, `reg_channel`, `imei` FROM `sys_login` WHERE `reg_channel`!='' AND DATE_FORMAT( `create_date`, '%Y%m' ) = DATE_FORMAT( CURDATE( ) , '%Y%m' )";
				$sql = "SELECT DATE_FORMAT( `create_date`, '%d' ) AS day, `reg_channel` FROM `sys_login` WHERE DATE_FORMAT( `create_date`, '%Y%m' ) = DATE_FORMAT( CURDATE( ) , '%Y%m' )";
				...
				foreach( $arrData as $data )
				{
					...
					// 	苹果
					if ( $output[0][0] == 'App Store' )
					{
						$num = $this->AdSpreadModel->getCount( array( 'imei'=>$data->imei ) );
						if ( $num > 0 )
						{
							$count = $rs['count4'][$key];
