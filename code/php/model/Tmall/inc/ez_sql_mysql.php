	...
	*/
	...
	class ezSQL_mysql extends ezSQLcore
	{
	   ...
	   * */
		 ...
		 * array( 'id'=>array('>' => 5) )				==============	  WHERE `id` > 5
		 * array( 'id'=>array('>' => 5, '<' => 8) )		==============	  WHERE `id` > 5 AND `id` < 8
		 *
		 * $arrCol: 要输出的列
		 *
		 * 实例：
	     ...
	     * */
		function get_list( $table, $arrWhere, $arrCol, $strOrderBy, $arrLimit )
		{
			$strWhere 	= "";
			$strCol   	= "*";
			$strLimit	= "";
			...
			if ( is_array( $arrWhere ) )
			{
				foreach ( $arrWhere as $key=>$val )
				{
					...
					if ( is_array($val)  )
					{
						...
						{
							$arrVal2[] = " AND `{$key}` $subKey '{$this->escape($subVal)}'";
						}
					}
					else
					{
			...
			if ( is_array( $arrLimit )  )
			{
				if ( count( $arrLimit ) == 2 )
				{
					$strLimit = "LIMIT {$arrLimit[0]},{$arrLimit[1]}";
				}
				else
				{
					$strLimit = "LIMIT {$arrLimit[0]}";
				}
			}

			$strSQL = "SELECT {$strCol} FROM `{$table}` WHERE 1=1 {$strWhere} {$strOrderBy} {$strLimit}";
