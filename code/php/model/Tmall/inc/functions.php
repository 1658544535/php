...
// 上传excel文件
function uploadexcelfile($uploader = 'file', $path = '../upfiles/')
{
	do
	{
		if ( $_FILES[$uploader]['size'] == '' )
		{
			$return = FALSE;
			break;
		}
...
function uh($str)
{
  ...
  return $str;
	...
	function get_qrcode_img($card_id,$type,$file_name='')
	{
		include "../tools/qrcode/phpqrcode/phpqrcode.php";
	 ...
	 *  @param $conn	数据库链接
	 */
// 	function M( $dbName, $conn='' )
// 	{
// 		require_once APP_INC . '/db.php';
// 		return new Db( $conn,$dbName);
// 	}
	 ...
	 * 功能:获取支付二维码
	 */
	function getQrCode( $fact_price, $out_trade_no, $order_id )
	{
		require_once('../wxpay/inc_clickfarm.php');
		$ClickFarm = new ClickFarm();
		$rs = $ClickFarm->getQrCode( $fact_price, $out_trade_no);

		if ( $rs['return_code'] == 'SUCCESS' && $rs['result_code'] == 'SUCCESS' )
		{
			get_qrcode_img( $rs['code_url'],'wxpay',$order_id);
			return $rs;
		}
		else
		{
