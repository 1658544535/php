...
class Model{
	 ...
	 *
	 */
	public function get($cond=array(), $fields='*', $order='', $outputType=OBJECT, $paramTable='')
	{
		$sql = $this->getSQL(  $cond, $fields, $order, '', $paramTable );
	...
	public function getAll($cond=array(), $order=array(), $output = OBJECT, $limit='',$fields='',$paramTable=''){
		$sql = $this->getSQL( $cond, $fields, $order, $limit, $paramTable );
		...
		if ( is_array( $where ) )
		{
			...
			foreach( $where as $key=>$val )
			{
				...
				else
				{
					...
					{
						if ( count( $val ) == 1 && is_numeric($subKey) )
						{
