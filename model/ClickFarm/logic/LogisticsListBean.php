...
class LogisticsListBean extends  Db
{
	 ...
	 */
	 public function addOrderShip( $order_id, $logistics_name, $logistics_no, $consignee, $consignee_address )
	 {
	 	// 1、通过order_id获取user_order记录
		...
		// 3、修改user_order的order_status
	 	$this->change_table( 'user_order' );
		$this->update( array('order_status'=>3,'consignee'=> $consignee, 'consignee_address'=>$consignee_address), array('id'=>$order_id));
	  ...
	  *	功能：清空快递列表中，没用的快递时间
	  */
	 public function updateLogisticsDate( $date )
	 {
	 	$this->update( array('date'=>''), array('date'=>$date) );
	 }

	 /**
	  *	功能：获取快递仓库的列表
	  */
	 public function getLogisticsWarehouse()
	 {
	 	return $this->get_list(  array('date'=>'0000-00-00') );
	 }
