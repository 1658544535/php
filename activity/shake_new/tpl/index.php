...
<title>摇一摇</title>
<style>
html,body{width:100%;height:100%;max-width:640px;margin:0 auto;padding:0;}
.wrapper{position:relative;width:100%;height:100%;background:#eb6da5;z-index:2;}
.award{background:#eb6da5;}
.bg_top,.bg_top img{display:block;width:100%;height:auto;border:none; margin-top:5px;}
.main>img{position:absolute;bottom:0;left:0;width:100%;height:auto;border:none;}
.bg_top{position:absolute;top:0;left:0;width:100%;height:30%;z-index:99;}
.main{position:absolute;width:100%;height:96%;bottom:4%;left:0;background:#fff;overflow:hidden;}
.shake{position:absolute;bottom:29%;left:0;width:100%;height:auto;transform-origin: 54.7% 100%;}
.shake img{display:block;width:100%;}
.myRecord{position:absolute;width:100%;bottom:2%;height:auto;z-index:3;}
.myRecord a{font-size:0;opacity:0;display:block;width:32%;margin:0 auto;padding-bottom:10%;background:#000;}
.next{position:absolute;left:48%;bottom:0;width:4%;height:2%;background:url(images/next.png) no-repeat;background-size:100% auto;-webkit-animation:next 1.5s infinite ease-in-out;-moz-animation:next 1.5s infinite ease-in-out;-ms-animation:next 1.5s infinite ease-in-out;-o-animation:next 1.5s infinite ease-in-out;animation:next 1.5s infinite ease-in-out;}
.award_top,.award_footer{display:block;width:100%;}
.award_list{width:100%;}
.award_list img{width:100%;margin-bottom:5px;}
...
#audio{position:absolute;z-index:-999;}


@-webkit-keyframes shake{0%,50%,100%{-webkit-transform:rotate(-20deg);}25%,75%{-webkit-transform:rotate(20deg);}}
@-moz-keyframes shake{0%,50%,100%{-moz-transform:rotate(-20deg);}25%,75%{-moz-transform:rotate(20deg);}}
@-ms-keyframes shake{0%,50%,100%{-ms-transform:rotate(-20deg);}25%,75%{-ms-transform:rotate(20deg);}}
@-o-keyframes shake{0%,50%,100%{-o-transform:rotate(-20deg);}25%,75%{-o-transform:rotate(20deg);}}
@keyframes shake{0%,50%,100%{transform:rotate(-20deg);}25%,75%{transform:rotate(20deg);}}
...
@-webkit-keyframes next{0%,30%{opacity:0;-webkit-transform:translate(0,-15px)}60%{opacity:1;-webkit-transform:translate(0,-5px)}100%{opacity:0;-webkit-transform:translate(0,0px)}}
@-moz-keyframes next{0%,30%{opacity:0;-moz-transform:translate(0,-15px)}60%{opacity:1;-moz-transform:translate(0,-5px)}100%{opacity:0;-moz-transform:translate(0,0px)}}
@-ms-keyframes next{0%,30%{opacity:0;-ms-transform:translate(0,-15px)}60%{opacity:1;-ms-transform:translate(0,-5px)}100%{opacity:0;-ms-transform:translate(0,0px)}}
@-o-keyframes next{0%,30%{opacity:0;-o-transform:translate(0,-15px)}60%{opacity:1;-o-transform:translate(0,-5px)}100%{opacity:0;-o-transform:translate(0,0px)}}
@keyframes next{0%,30%{opacity:0;transform:translate(0,-15px)}60%{opacity:1;transform:translate(0,-5px)}100%{opacity:0;transform:translate(0,0px)}}

.animateShake{-webkit-animation: shake 4s infinite;-moz-animation: shake 4s infinite;-ms-animation: shake 4s infinite;-o-animation: shake 4s infinite;animation: shake 4s infinite;}
...
<body>
	...
	<div class="wrapper">
	    <div class="bg_top">
	        <img src="images/bg-top.png" alt="摇一摇送福利">
	    </div>
	    <!--头部-->
	    <div class="main">
	...
	<?php if ( $rs != null ){ ?>
		<div class="award">
			<img src="images/award-top.png" class="award_top" />
	        ...
	        <img src="images/award-footer.png" class="award_footer" />
	    </div>
    <?php } else{  ?>
    	<div class="award">
			<img src="images/award-top.png" class="award_top" />
...
<script>
    ...
    function popup(state,imgSrc,proName)
    {
        if( state == 200 )
        {
            ...
            btn = '<a href="<?php $site ?>index.php?act=record" class="btn_see">查看中奖记录</a>';
        }
        else if( state == -101 )
        {
        	//活动结束
	...
	function get_result()
	{
		$.ajax({
			...
			success:function(data){
				if ( data.code == -100 )
				{
					location.href="<?php echo $site ?>reg.php";
				}
				else if( data.code == -101 )
				{
					popup( data.code, '', data.msg);			// 弹窗
				}
				else
				{
